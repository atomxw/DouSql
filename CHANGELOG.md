...
### 新增功能
- **新增6个专业payload组**：总计300+个专业SQL注入payload
  - `blind-injection-fuzz`（42个）：盲注专用字典，支持MySQL/MSSQL/PostgreSQL
  - `login-password-injection-fuzz`（73个）：登录绕过专用字典
  - `mssql-payloads-fuzz`（14个）：MSSQL数据库专用payload
  - `oracle-payloads-fuzz`（8个）：Oracle数据库专用payload
  - `union-select-bypass`（30个）：UNION注入WAF绕过字典
- **更新default组**：优化为21个高质量通用payload
- **新增payload组文档**：添加PAYLOAD_GROUPS.md详细说明文档

### 修复
- **修复空格URL编码问题**：空格编码功能现在独立工作，不再依赖"自定义payload"开关
  - 修复了`processCustomPayload`方法的逻辑错误
  - 只要勾选"空格url编码"，payload中的空格就会被正确编码为%20
  - 更新UI文本，让功能更清晰
- **修复长度差异检测问题**：长度差异检测现在使用原始响应长度作为基准
  - 之前的逻辑依赖特定payload（'、''、-1、-0）来设置基准，导致很多情况下无法检测
  - 现在直接使用原始请求的响应长度作为基准，所有payload都能正确检测长度差异
  - 只要响应长度差异超过配置的阈值，就会标记为diff
- **修复状态显示问题**：改进状态更新逻辑，支持同时显示多个检测结果
  - 之前当同时检测到err和diff时，只显示err（高优先级覆盖低优先级）
  - 现在会累积显示，如同时检测到err和diff，会显示`err+diff`
  - 测试结束时会显示所有检测到的异常类型
- **增强调试日志**：添加详细的payload编码处理和长度差异检测调试信息
  - 显示编码前后的payload对比
  - 显示原始长度、当前长度、差异值和阈值
  - 显示传递给Burp API的实际值
  - 添加方法执行流程标记，便于排查问题
...
### 优化改进
- **优化payload加载逻辑**：支持从配置文件加载时跳过注释行（#开头）
- **完善README文档**：更新payload组说明，添加使用建议和测试策略
- **新增测试文档**：添加TEST_PAYLOADS.md测试验证指南
