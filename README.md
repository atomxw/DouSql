...
### UI显示优化
插件针对Windows系统进行了专门优化，解决了常见的UI显示问题：

- **payload组管理按钮**：重新设计布局，确保"新建"、"重命名"、"删除"按钮完整显示
- **白名单功能按钮**：优化GridBagLayout约束，解决"启动白名单"按钮被隐藏的问题
- **DPI适配**：自动适配高分辨率显示器，确保界面清晰
- **字体渲染**：优化字体渲染效果，提升可读性

### 常见问题解决
1. **按钮显示不完整**：
   - 插件会自动检测Windows系统并应用UI优化
   - 如仍有问题，可尝试调整Burp Suite的JVM参数：`-Dsun.java2d.uiScale=1.0`

2. **白名单输入框被隐藏**：
   - 已重新设计控制面板布局，确保输入框正确显示
   - 输入框现在有明显的边框，便于识别和点击

3. **界面出现空白区域**：
   - 已优化分割面板布局，避免重复布局导致的空白
   - 控制面板和标签页现在使用统一的布局结构

4. **界面模糊**：
   - 插件已自动启用DPI感知
   - 确保Windows系统显示缩放设置合理（建议100%-150%）

5. **组件重叠**：
   - 使用改进的布局管理器，避免组件重叠
   - 右侧面板最小宽度设置为250像素，确保所有组件可见

## JSON参数处理

插件对JSON参数提供了强大的支持，能够处理各种复杂的JSON结构：

### 支持的JSON结构
- **简单键值对**：`{"username": "test", "password": "123"}`
- **嵌套对象**：`{"user": {"profile": {"name": "test"}}}`
- **数组结构**：`{"items": [1, 2, 3], "users": [{"id": 1}, {"id": 2}]}`
- **混合结构**：包含对象、数组、字符串、数字、布尔值的复杂JSON

### 智能替换策略
1. **简单替换**：优先使用正则表达式进行快速替换
2. **深度遍历**：对于复杂结构，使用递归算法深度遍历所有字段
3. **类型保持**：自动识别并保持原始数据类型（字符串、数字、布尔值等）
4. **结构完整性**：确保JSON结构在替换后保持有效
...
// 插件会自动找到并测试所有字段：
// - user.profile.settings.security.password
